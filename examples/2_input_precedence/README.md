...
```console
$ # change the schema name defined in `postgre.yaml` to `otherdb`
$ uv run myapp_argparse.py --config postgre.yaml --schema_name otherdb
PostgreSQLConfig(host='example.com', port=5432, schema_name='otherdb')
```
-->
...
```console
$ # change the schema name defined in `postgre.yaml` to `otherdb`
$ MYAPP_SCHEMA_NAME=otherdb uv run myapp_argparse.py --config postgre.yaml
PostgreSQLConfig(host='example.com', port=5432, schema_name='otherdb')
```
-->
...
```console
$ # change the schema name defined in `postgre.yaml` to `otherdb`
$ MYAPP_SCHEMA_NAME=otherdb uv run myapp_argparse.py --config postgre.yaml --schema_name=finaldb
PostgreSQLConfig(host='example.com', port=5432, schema_name='finaldb')
```
-->
...
<!--
```console
$ uv run myapp_argparse.py --config postgre_no_schema.yaml --schema_name mydb
PostgreSQLConfig(host='dev.example.com', port=5555, schema_name='mydb')
```
-->
...
<!--
```console
$ uv run myapp_argparse.py --config postgre.yaml postgre_no_schema.yaml
PostgreSQLConfig(host='dev.example.com', port=5555, schema_name='mydb')
```
-->
...
<!--
```console
$ uv run myapp_argparse.py --config postgre_no_schema.yaml postgre.yaml
PostgreSQLConfig(host='example.com', port=5555, schema_name='mydb')
```
-->
...
$ # The port specified on the command line overwrites that of the config even if
$ # the config is specified after
$ uv run myapp_argparse.py --port 6006 --config postgre.yaml
PostgreSQLConfig(host='example.com', port=6006, schema_name='mydb')
```
-->
